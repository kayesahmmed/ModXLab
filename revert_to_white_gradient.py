...
# 1. Contact Me Button -> Revert to White + Cyan gradient mix
...
# 2. Search Bar Background -> Revert to gradient mix
old_search_bg = """                <div 
...
# 3. Search Input Text -> Back to Dark
old_search_input = """className="w-full h-full bg-transparent text-[16px] outline-none border-none shadow-none pl-5 pr-10 font-['Plus_Jakarta_Sans',sans-serif] text-white placeholder-white/50 font-medium\""""
...
# 4. Search Input close button -> Back to dark
...
# 5. Search Toggle Button (the icon button) -> Back to gradient mix
...
# 6. Suggestions box match white theme
old_sugg_bg = """                    <div className="absolute inset-0 rounded-2xl pointer-events-none z-0 bg-slate-900/95 backdrop-blur-3xl border border-[#00E5D1]/30 shadow-[0_24px_60px_-10px_rgba(0,229,209,0.15)] ring-1 ring-white/10" />"""
new_sugg_bg = """                    <div className="absolute inset-0 rounded-2xl pointer-events-none z-0 bg-white/95 backdrop-blur-3xl border border-[#00E5D1]/40 shadow-[0_24px_60px_-10px_rgba(0,229,209,0.25)] ring-1 ring-black/5" />"""
content = content.replace(old_sugg_bg, new_sugg_bg)

# 7. Suggestions container text color -> Dark
old_sugg_text = """                    <div className="relative z-10 w-full h-full text-white">"""
new_sugg_text = """                    <div className="relative z-10 w-full h-full text-slate-900">"""
content = content.replace(old_sugg_text, new_sugg_text)

# 8. Suggestions item hover effect -> Match white theme
old_item_hover = """                          className="w-full text-left p-3 rounded-xl transition-all duration-300 flex items-center justify-between gap-3 cursor-pointer group hover:bg-[#00E5D1]/10 active:scale-[0.98] text-white border border-transparent hover:border-[#00E5D1]/30 relative overflow-hidden\""""
new_item_hover = """                          className="w-full text-left p-3 rounded-xl transition-all duration-300 flex items-center justify-between gap-3 cursor-pointer group hover:bg-[#00E5D1]/15 active:scale-[0.98] text-slate-900 border border-transparent hover:border-[#00E5D1]/30 relative overflow-hidden\""""
content = content.replace(old_item_hover, new_item_hover)

# 9. Icons
old_icon_1 = """                                <img src={item.imageUrl} className="w-10 h-10 rounded-xl object-cover shadow-sm ring-1 ring-white/10 group-hover:ring-[#00E5D1]/50 transition-all duration-300" alt="Icon" />"""
new_icon_1 = """                                <img src={item.imageUrl} className="w-10 h-10 rounded-xl object-cover shadow-sm ring-1 ring-black/5 group-hover:ring-[#00E5D1]/50 transition-all duration-300" alt="Icon" />"""
content = content.replace(old_icon_1, new_icon_1)

old_icon_2 = """                              <div className="w-10 h-10 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center shrink-0 text-white group-hover:bg-[#00E5D1]/20 group-hover:border-[#00E5D1]/50 group-hover:text-[#00E5D1] transition-all duration-300 shadow-sm">"""
new_icon_2 = """                              <div className="w-10 h-10 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0 text-slate-500 group-hover:bg-[#00E5D1]/20 group-hover:border-[#00E5D1]/50 group-hover:text-[#00E5D1] transition-all duration-300 shadow-sm">"""
content = content.replace(old_icon_2, new_icon_2)

# 10. Item Title/Desc
old_title = """                              <span className="font-extrabold text-sm truncate group-hover:text-[#00E5D1] transition-colors text-white/95">"""
new_title = """                              <span className="font-extrabold text-sm truncate group-hover:text-[#00E5D1] transition-colors text-slate-900">"""
content = content.replace(old_title, new_title)

old_desc = """                              <span className="text-[11px] font-medium mt-0.5 truncate text-white/50 group-hover:text-white/70">"""
new_desc = """                              <span className="text-[11px] font-medium mt-0.5 truncate text-slate-500 group-hover:text-slate-700">"""
content = content.replace(old_desc, new_desc)

# 11. Badge
old_badge = """                          <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md bg-white/10 text-white/70 border border-white/10 shrink-0 group-hover:border-[#00E5D1]/40 group-hover:bg-[#00E5D1]/20 group-hover:text-[#00E5D1] transition-all duration-300 relative z-10">"""
new_badge = """                          <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md bg-slate-100 text-slate-500 border border-slate-200 shrink-0 group-hover:border-[#00E5D1]/40 group-hover:bg-[#00E5D1]/20 group-hover:text-[#00E5D1] transition-all duration-300 relative z-10">"""
content = content.replace(old_badge, new_badge)

# 12. No matches text
old_nomatch = """                    <div className="p-4 text-center flex items-center justify-center gap-2 text-xs font-semibold text-white/90">"""
new_nomatch = """                    <div className="p-4 text-center flex items-center justify-center gap-2 text-xs font-semibold text-slate-700">"""
